...
class Seq2Seq(object):
    ...
    def __init__(self, xseq_len, yseq_len, 
            xvocab_size, yvocab_size, id2word,
            emb_dim, num_layers, ckpt_path,
            lr=0.0001, 
            epochs=2, model_name='seq2seq_model'):

        # attach these arguments to selfnp
    ...
    def get_feed(self, X, Y, keep_prob):
        ...
        for t in range(self.xseq_len):
            feed_dict[ self.enc_ip[t] ] =  X[t]
        for t in range(self.yseq_len):
            feed_dict[ self.labels[t] ] =  Y[t]
    ...
    def train(self, train_set, valid_set, sess=None ):
        ...
        for i in range(self.epochs):
            try:
                ...
                if i and i% (self.epochs//1) == 0: # TODO : make this tunable by the user
                    # save model to disk
    ...
    def eval_step(self, sess, eval_batch_gen):
        ...
        # print( np.array(dec_op_v).shape ) 
        # dec_op_v = np.array(dec_op_v).transpose([1,0,2])
        # print( dec_op_v.shape ) 
        return loss_v, dec_op_v
    ...
    def eval_batches(self, sess, eval_batch_gen, num_batches):
        ...
        for i in range(num_batches):
            print ("eval step %d/%d"%(i+1, num_batches))
            loss_v, dec_op_v = self.eval_step(sess, eval_batch_gen)
            loss_list.append(loss_v)
            result = np.argmax(dec_op_v, axis=2)
            # print( result.shape )
            # print( data_utils.decode(result, self.id2word) )    # 주석 해체하면 결과 텍스트가 나온다
        return np.mean(loss_list)

    # prediction
    def predict(self, sess, test_batch_gen, batch_size):
        print( "predict start")
        loss_mean = self.eval_batches(sess,test_batch_gen, batch_size)
        return loss_mean
